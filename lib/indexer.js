...
var _defaultIndexer = function(item, options) {
  ...
  if (!options.update) {
    index.index.op_type = 'create';
  }

  if (item._routing) {
    index.index.routing = item._routing;
  }
    ...
    options.client.bulk({
      ...
      if (res.errors) {
        res.items.forEach(function(item) {
          if (item.create && _.indexOf([200, 201, 409], item.create.status) == -1) {
            self.emit('item-failed', item);
          }
          if (item.index && _.indexOf([200, 201], item.index.status) == -1) {
            self.emit('item-failed', item);
          }
